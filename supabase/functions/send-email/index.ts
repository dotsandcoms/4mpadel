...
function generateEmailBody(template: string, vars: any): { subject: string; html: string } {
  ...
  switch (template) {
    case 'broadcast':
      ...
      contentHtml = `<div style="font-size: 14px; line-height: 1.6; color: #E2E8F0; white-space: pre-wrap;">${vars.message || ''}</div>`;
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (finalBcc) {
        ...
        // If we only have BCC (like in a broadcast), Resend requires a 'To' address. We can use the fromAddress or a dummy address.
        if (!payload.to) {
